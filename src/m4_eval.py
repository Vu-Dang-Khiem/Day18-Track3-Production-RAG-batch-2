...
from config import TEST_SET_PATH, get_ragas_llm, get_ragas_embeddings, LLM_PROVIDER
...
def failure_analysis(eval_results: list[EvalResult], bottom_n: int = 10) -> list[dict]:
    ...
    for r in eval_results:
        metrics = {"faithfulness": r.faithfulness, "context_recall": r.context_recall,
                   "context_precision": r.context_precision, "answer_relevancy": r.answer_relevancy}
        avg_score = sum(metrics.values()) / len(metrics)
        worst_metric = min(metrics, key=metrics.get)
        diagnosis, fix = diagnostic_tree.get(worst_metric, ("Unknown", "Review pipeline"))
        scored.append({
            "question": r.question,
